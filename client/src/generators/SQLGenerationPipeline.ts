...
export class SQLGenerationPipeline {
   ...
   */
 // ==================== ADD THESE HELPER METHODS SOMEWHERE IN THE CLASS ====================

/**
 * Check if a node name was auto‑generated by the system
 */
private isAutoGeneratedName(name: string): boolean {
  const autoPatterns = [
    ...
    /^output_\d+$/,
    /^.*_OUTPUT_\d+$/i,          // matches names like excel-file_OUTPUT_1
    /^cte_[a-f0-9]+_/i,
    /^tmp_[a-f0-9]+_/i,
  ];
...
/**
 * Validate that all output nodes have user‑defined names.
 * Throws an error if any output node uses an auto‑generated name.
 */
private validateUserDefinedOutputNames(graph: DependencyGraph): void {
  const outputNodes = Array.from(graph.nodes.values()).filter(node => this.isOutputNode(node));
  const invalidNodes = outputNodes.filter(node => this.isAutoGeneratedName(node.name));

  if (invalidNodes.length > 0) {
    const nodeList = invalidNodes.map(n => `"${n.name}" (id: ${n.id})`).join(', ');
    throw new Error(
...
// ==================== THE COMPLETE generate METHOD ====================

public async generate(): Promise<PipelineGenerationResult> {
  this.generationStartTime = Date.now();
  this.logger.info('Starting pipeline generation');

  // --- ALWAYS LOG PIPELINE INPUT TO BROWSER CONSOLE (unconditional) ---
  console.log('=== PIPELINE GENERATION INPUT (ALWAYS) ===');
  console.log('Nodes (count: %d):', this.nodes.length);
  for (let i = 0; i < this.nodes.length; i++) {
    const node = this.nodes[i];
    console.log(`  ${i}. ${node.id} (${node.type}) name="${node.name}" metadata keys: ${Object.keys(node.metadata || {})}`);
  }
  console.log('Connections (count: %d):', this.connections.length);
  for (let i = 0; i < this.connections.length; i++) {
    const conn = this.connections[i];
    console.log(`  ${i}. ${conn.id}: ${conn.sourceNodeId} → ${conn.targetNodeId} (status: ${conn.status})`);
  }
  console.log('Options:', JSON.stringify(this.options, (key, value) =>
  ...
  try {
    ...
    // --- NEW: Validate output node names ---
  ...
  // For any fragment that contains a SELECT (including those without FROM)
  const selectMatch = fragment.sql.match(/SELECT\s+(.*?)(?:\s+FROM|$)/is);
  if (selectMatch) {
    const selectClause = selectMatch[1];
    const cleaned = selectClause.replace(/\s+/g, ' ').trim();
    // Split by commas (assumes no commas inside function arguments – adjust if needed)
    const columns = cleaned.split(',').map(col => {
  ...
  // Fallback to node's metadata schema
  return this.extractNodeSchema(node);
}
...
private extractSelectColumns(sql: string): string[] {
  // Match SELECT clause up to FROM or end of string, with 's' flag for newlines
  const selectMatch = sql.match(/SELECT\s+(.*?)(?:\s+FROM|$)/is);
  if (!selectMatch) return [];
  const selectClause = selectMatch[1];
  const cleanedClause = selectClause.replace(/\s+/g, ' ').trim();
  // Match column names with optional AS aliases
  const columnPattern = /(\w+)\s+AS\s+(\w+)|(\w+)(?=\s*[,)]|$)/gi;
  const columns: string[] = [];
  let match: RegExpExecArray | null;
  while ((match = columnPattern.exec(cleanedClause)) !== null) {
    if (match[2]) {
      columns.push(match[2]);      // alias after AS
    } else if (match[3]) {
      columns.push(match[3]);      // simple column name
    }
  }
  return columns;
}
...
private async buildFinalSQL(
  ...
  // Build mapping nodeId -> CTE name (include all nodes, but skip output in CTE list)
  ...
  // Modify each fragment to replace upstream node IDs with their CTE names
  ...
  // Identify output node (first node with type OUTPUT)
  ...
  // Find predecessors of output node (should be at least one)
  ...
  if (!sourceCteName) {
    throw new Error(`No CTE generated for source node ${sourceNodeId}`);
  }

  // Get the source fragment and extract its output columns directly
  const sourceFragment = modifiedFragments.get(sourceNodeId);
  if (!sourceFragment) {
    throw new Error(`No fragment found for source node ${sourceNodeId}`);
  }
  const sourceColumns = this.extractSelectColumns(sourceFragment.sql);
  if (sourceColumns.length === 0) {
    throw new Error('Could not determine source columns from tMap fragment');
  }

  // Get target table name from output node (use output node's name or configuration)
  const targetTable = this.getTargetTableName(outputNode);
  const sanitizedTargetTable = this.sanitizeIdentifier(targetTable);
  const insertCols = sourceColumns.map(c => this.sanitizeIdentifier(c)).join(', ');
  const selectCols = sourceColumns.map(c => this.sanitizeIdentifier(c)).join(', ');

  // Build CTE definitions, skipping output nodes
  const cteDefinitions: string[] = [];
  for (const nodeId of topologicalOrder) {
    const node = graph.nodes.get(nodeId);
    if (!node) continue;
    if (this.isOutputNode(node)) continue; // skip output nodes
  ...
  finalSQL += `INSERT INTO ${sanitizedTargetTable} (${insertCols})\nSELECT ${selectCols} FROM ${sourceCteName};`;
