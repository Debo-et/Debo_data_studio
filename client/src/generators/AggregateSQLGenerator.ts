...
interface SQLGenerationError {
  ...
  field?: string;  // Make this optional
}
...
interface SourceColumn {
  ...
  isPrimaryKey?: boolean;  // Make optional
  isUnique?: boolean;      // Make optional
}
...
export class AggregateSQLGenerator extends BaseSQLGenerator {
  ...
  protected generateSelectStatement(context: SQLGenerationContext): GeneratedSQLFragment {
    const { node } = context;
    const aggConfig = node.metadata?.aggregationConfig;
  ...
  protected generateJoinConditions(_context: SQLGenerationContext): GeneratedSQLFragment {
    // AGGREGATE nodes typically don't have JOIN conditions
    return this.emptyFragment('join_conditions');
  }

  protected generateWhereClause(_context: SQLGenerationContext): GeneratedSQLFragment {
    // AGGREGATE nodes use HAVING, not WHERE for aggregate filtering
  ...
  private generateWindowAggregation(
    ...
    const nonGroupingColumns = sourceColumns.filter(col => 
      !aggConfig.groupBy.includes(col.name)
    );
  ...
  private generateTraditionalAggregation(
    ...
    aggConfig.aggregates.forEach(agg => {
      const distinctClause = options.distinctAggregates ? 'DISTINCT ' : '';
      selectColumns.push(`${agg.function}(${distinctClause}${this.sanitizeIdentifier(agg.column)}) AS ${this.sanitizeIdentifier(agg.alias)}`);
    });
  ...
  private generateAggregationPerformanceHints(
    ...
    aggConfig.groupBy.forEach(column => {
      ...
      // Use optional chaining since isPrimaryKey and isUnique are optional
  ...
  private validateAggregationConfig(
    ...
    aggConfig.groupBy.forEach(column => {
      if (!sourceColumnNames.has(column)) {
        errors.push({
          ...
          severity: 'ERROR'
          // Removed field property as it's not in SQLGenerationError type
        });
      }
    });
    ...
    aggConfig.aggregates.forEach(agg => {
      ...
      if (agg.column !== '*') {
        ...
        if (sourceCol) {
          const isCompatible = this.validateAggregateCompatibility(
            agg.function,
            sourceCol.dataType
          );
          
          if (!isCompatible) {
            warnings.push(`Aggregate function ${agg.function} may not be compatible with ${sourceCol.dataType} column "${agg.column}"`);
          }
        }
      }
    });
    ...
    if (aggConfig.having) {
      ...
      if (!validation.valid) {
        errors.push({
          ...
          message: validation.error || 'Invalid HAVING clause', // Provide default message
          severity: 'ERROR'
          // Removed field property
        });
      }
  ...
  private validateAggregateCompatibility(
    functionName: string,
    dataType: PostgreSQLDataType
  ): boolean {
    const numericTypes = [
  ...
  private validateHavingClause(
    ...
    const allowedColumns = [
      ...aggConfig.groupBy,
      ...aggConfig.aggregates.map(agg => agg.alias)
    ];
  ...
  private generateAggregateSelect(aggConfig: AggregationConfig): string {
    ...
    // Add aggregate functions
    aggConfig.aggregates.forEach(agg => {
      columns.push(`${agg.function}(${agg.column === '*' ? '*' : this.sanitizeIdentifier(agg.column)}) AS ${this.sanitizeIdentifier(agg.alias)}`);
    });
