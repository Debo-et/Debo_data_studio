/**
 * Enhanced HSQLDB Schema Inspector with Query Execution
...
interface DatabaseConfig {
  ...
  port?: string;            // Port (default: '9001' for hsql protocol)
  user?: string;            // Username
  password?: string;        // Password
  schema?: string;          // Target schema (default: 'PUBLIC')
  protocol?: string;        // 'hsql', 'http', 'https' (default: 'hsql')
}
...
class HsqlDBConnectionError extends Error {
  constructor(
    ...
    super(message);
    this.name = 'HsqlDBConnectionError';
  }
}

class HsqlDBQueryError extends Error {
  constructor(
    message: string,
    public sql: string,
...
class HsqlDBConnectionManager {
  ...
  async query(sql: string, params: any[] = []): Promise<any> {
    ...
    if (!this.isConnected || !this.connection) {
      throw new HsqlDBConnectionError('Database not connected');
    }
    ...
    try {
      ...
      const queryError = new HsqlDBQueryError(
   ...
   * HSQLDB expects literals; strings are single-quoted with doubled internal quotes.
   */
  ...
  private validateQueryParameters(sql: string, params: any[] = []): void {
    if (!sql || typeof sql !== 'string') {
      throw new HsqlDBQueryError('SQL query must be a non-empty string', sql, params);
    }
...
class HsqlDBSchemaInspector {
  ...
  /**
   * Executes arbitrary SQL query against the HSQLDB database
   */
  async executeQuery(
    sql: string,
    params: any[] = [],
  ...
  /**
   * Apply row limit to SELECT queries (HSQLDB supports FETCH FIRST n ROWS ONLY)
   */
  ...
  /**
   * Enhanced column retrieval from INFORMATION_SCHEMA.COLUMNS
   */
  private async getTableColumns(table: TableInfo): Promise<void> {
    const query = `
      SELECT
        ...
        COLUMN_DEFAULT,
        REMARKS AS comment,
        CASE WHEN COLUMN_TYPE LIKE '%IDENTITY%' THEN TRUE ELSE FALSE END AS is_identity
  ...
  /**
   * Enrich table with row count (attempted via approx, else set to 0)
   */
  private async enrichTableMetadata(table: TableInfo): Promise<void> {
    try {
      // HSQLDB does not expose approximate row counts easily; we set size/rowCount to N/A
      table.size = 'N/A';
      table.rowCount = 0;
    } catch (error) {
   ...
   */
  getConnection(): HsqlDBConnectionManager {
...
export {
  HsqlDBSchemaInspector,
  HsqlDBConnectionManager as HsqlDBConnection,
  TableInfo,
  Logger,
  HsqlDBConnectionError,
  HsqlDBQueryError,
  getErrorMessage,
  isError
};
...
export default HsqlDBSchemaInspector;