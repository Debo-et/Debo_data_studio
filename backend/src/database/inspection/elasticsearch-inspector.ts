...
class ElasticsearchConnection {
  ...
  private async executeDSLQuery(dsl: string): Promise<any> {
    ...
    // total can be a number or an object like { value: number, relation: string }
    const totalHits = response.hits.total;
    const rowCount = typeof totalHits === 'number' ? totalHits : (totalHits as any)?.value ?? 0;
    ...
    return {
      ...
      rowCount,
      fields: [],  // DSL response does not directly provide a schema
