...
export class MaxDBSchemaInspector {
  ...
  private async getTableColumns(tableName: string): Promise<ColumnMetadata[]> {
    ...
    return columns.map((col: any, idx: number) => ({
      ...
      comment: undefined,          // fixed: null -> undefined
      length: undefined,           // fixed: null -> undefined
      precision: undefined,        // fixed: null -> undefined
      scale: undefined,            // fixed: null -> undefined
  ...
  async executeQuery(
    ...
    try {
      ...
      // Use type assertion to include the extra 'duration' field
      return {
        success: true,
        rows: rows,
